# Spring AI Support Assistant — Hands-On Walkthrough

Follow this README top to bottom: generate a fresh Spring Boot app, paste the snippets as you go, and after every step you'll have something you can hit with `curl`. Each step is a small change to a single file. You'll never need to scroll back to look at the "final" code.

## 0. Generate the project

The walkthrough uses **OpenAI** as the example provider throughout. If you have access to a different one, change the `PROVIDER` variable below — `anthropic`, `ollama`, or `bedrock-converse` — and the Initializr will pull the matching starter. You'll then also need to swap the `spring.ai.openai.*` keys in the next section for your provider's namespace.
